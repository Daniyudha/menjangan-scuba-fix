...
class MyUploadAdapter {
    ...
    constructor(loader: CKEditorLoader) {
        this.loader = loader;
        this.apiUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080/api'}/gallery/ckeditor`;
    }
    ...
    upload() {
        return this.loader.file
            .then((file: File) => new Promise((resolve, reject) => {
                ...
                .then(res => {
                    if (!res.ok) {
                        return res.json().then(data => reject(data.error?.message || 'Server error during upload.'));
                    }
                    return res.json();
                })
                .then(data => {
                    if (data && data.url) {
                        resolve({
                            default: `${(process.env.NEXT_PUBLIC_API_URL || '').replace('/api', '')}${data.url}`
                        });
                    } else {
