...
export default function SettingsPage() {
    ...
    const handleHeroFileChange = async (e: ChangeEvent<HTMLInputElement>, slideId: string) => {
        ...
        try {
            ...
            // Perbarui state 'settings' secara langsung dengan URL baru dari backend
            setSettings(prev => {
            ...
            setNotification(`Image for slide updated successfully!`);

        } catch (error: unknown) {
            setNotification(`Error: ${error instanceof Error ? error.message : 'An unknown error occurred'}`);
        } finally {
            setUploading(prev => ({ ...prev, [slideId]: false })); // Selesai loading
        }
    };
    ...
    const handleSaveExperience = async (e: FormEvent) => {
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/settings/experience`, {
                method: "PUT",
                body: formData,
                credentials: 'include',
            });
            if (!response.ok) throw new Error("Failed to save Experience settings.");
                                ...
                                {settings.hero?.map((slide, index) => (
                                    <div key={slide.id || index} className="p-4 border-l-4 border-bright-blue bg-light-gray rounded-r-md">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div>
                                                <label className="block text-sm font-medium text-slate mb-1">Background Image</label>
                                                ...
                                                {uploading[slide.id] && <p className="text-xs text-slate animate-pulse">Uploading...</p>}
                        ...
                        <form onSubmit={handleSaveExperience} className="space-y-6">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full text-slate border border-slate/30 rounded-md file:mr-4 file:py-2 file:px-4 file:border-0 file:bg-gray-200 file:text-sm file:font-semibold file:bg-light-gray file:text-gray-blue hover:file:bg-slate/20"
                                />
                                <div className="mt-6 text-xs text-slate">
                                    Current image preview:
                                </div>
